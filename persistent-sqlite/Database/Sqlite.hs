...
instance Show SqliteException where
    show (SqliteException error functionName details) =
        unpack $
            Data.Monoid.mconcat
                [ "SQLite3 returned "
                , pack $ show error
...
openError path' = do
    let
        flag = sqliteFlagReadWrite .|. sqliteFlagCreate .|. sqliteFlagUri
        ...
        ( \(parameterIndex, datum) -> do
            case datum of
                ...
                PersistRational rational ->
                    bindText statement parameterIndex $ pack $ show (fromRational rational :: Pico)
                PersistBool b ->
                    bindInt64 statement parameterIndex $
                        if b then 1 else 0
...
status verb reset' = alloca $ \pCurrent -> alloca $ \pHighwater -> do
    let
        (code, hasCurrent, hasHighwater) = statusVerbInfo verb
    e <- decodeError <$> statusC code pCurrent pHighwater (if reset' then 1 else 0)
    case e of
        ErrorOK -> do
            current <-
                if hasCurrent then Just . fromIntegral <$> peek pCurrent else return Nothing
            highwater <-
                if hasHighwater then Just . fromIntegral <$> peek pHighwater else return Nothing
