...
runConn f = do
    let
        printDebug = when _debugOn . B.putStrLn . fromLogStr
...
spec = describe "persistent-qq" $ do
    ...
    it "sqlQQ/order-proof" $ db $ do
        ...
        ret1 <- runQuery
        ret2 <-
            runQuery :: (MonadIO m) => SqlPersistT m [Entity (ReverseFieldOrder Person)]
