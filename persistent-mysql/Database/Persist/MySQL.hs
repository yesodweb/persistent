...
-- @since 2.12.1.0
openMySQLConn
    :: MySQL.ConnectInfo -> LogFunc -> IO (MySQL.Connection, SqlBackend)
...
prepare' conn sql = do
    let
        query = MySQL.Query (T.encodeUtf8 sql)
    return
        Statement
            { stmtFinalize = return ()
            , stmtReset = return ()
...
withStmt' conn query vals = do
  ...
  where
    ...
    fetchRows result = liftIO $ do
        ...
        let
            getters = [maybe PersistNull (getGetter f f . Just) | f <- fields]
            convert = use getters
              where
                use (g : gs) (col : cols) =
                    let
...
migrate' connectInfo allDefs getter val = do
    let
        name = getEntityDBName val
    let
        (newcols, udefs, fdefs) = mysqlMkColumns allDefs val
        ...
        ([], [], _) -> do
            ...
            let
                foreigns = do
                    ...
                    let
                        refConstraintName = crConstraintName cRef
                    let
                        refTblName = crTableName cRef
                    let
                        refTarget =
  ...
  where
    findTypeAndMaxLen tblName col =
        let
            (col', ty) = findTypeOfColumn allDefs tblName col
...
findMaxLenOfColumn
    :: [EntityDef] -> EntityNameDB -> FieldNameDB -> (FieldNameDB, Integer)
...
data AlterTable
    = AddUniqueConstraint ConstraintNameDB [(FieldNameDB, FieldType, Integer)]
...
getColumns connectInfo getter def cols = do
    ...
    inter2 <-
        with (stmtQuery stmtClmns vals) (\src -> runConduitRes $ src .| CL.consume)
    ...
    us <-
        with (stmtQuery stmtCntrs vals) (\src -> runConduitRes $ src .| helperCntrs)
  ...
  where
    ...
    helperClmns = CL.mapM getIt .| CL.consume
      where
        getIt row =
            fmap (either Left (Right . Left))
                . liftIO
                . getColumn connectInfo getter (getEntityDBName def) row
                $ ref
          where
            ref = case row of
    ...
    helperCntrs = do
        let
            check
                [ PersistText cntrName
                    , PersistText clmnName
    ...
    cRef =
      ...
      where
        ...
        getRef (Just refName') = do
            ...
            cntrs <-
                liftIO $ with (stmtQuery stmt vars) (\src -> runConduit $ src .| CL.consume)
            pure $ case cntrs of
                [] ->
                    Nothing
                [ [ PersistText tab
...
parseColumnType
    :: Text -> ColumnInfo -> ExceptT String IO (SqlType, Maybe Integer)
-- Ints
...
parseColumnType "int" ci
    | ciColumnType ci == "int" || ciColumnType ci == "int(11)" =
        return (SqlInt32, Nothing)
parseColumnType "bigint" ci
    | ciColumnType ci == "bigint" || ciColumnType ci == "bigint(20)" =
        return (SqlInt64, Nothing)
-- Double
...
parseColumnType _ ci =
    return (SqlOther (ciColumnType ci), Nothing)
...
getAlters allDefs edef (c1, u1) (c2, u2) =
  ...
  where
    ...
    getAltersC (new : news) old =
        let
            (alters, old') = findAlters edef allDefs new old
         in
    ...
    case filter ((name ==) . cName) cols of
        ...
        Column _ isNull' type_' def' gen' _defConstraintName' maxLen' ref' : _ ->
            let
                ...
                modType
                    | showSqlType type_ maxLen False `ciEquals` showSqlType type_' maxLen' False
                        && isNull == isNull' =
                        []
             ...
             in
                ( refDrop ++ modType ++ modDef ++ modGen ++ refAdd
...
showColumn showReferences (Column n nu t def gen _defConstraintName maxLen ref) =
    concat
        [ escapeF n
        , " "
...
showSqlType
    ...
    -- ^ @maxlen@
    -> Bool
    -- ^ include character set information?
    -> String
...
showAlterTable table (DropUniqueConstraint cname) =
    concat
        [ "ALTER TABLE "
        , escapeE table
        , " DROP INDEX "
...
showAlter table (AddReference reftable fkeyname t2 id2 fc) =
    concat
        [ "ALTER TABLE "
        , escapeE table
        , " ADD CONSTRAINT "
...
instance PersistConfig MySQLConf where
    ...
    applyEnv conf = do
        env <- getEnvironment
        let
            maybeEnv old var = maybe old id $ lookup ("MYSQL_" ++ var) env
        return
            conf
                { myConnInfo =
                ...
                foreigns = do
                    Column
                        ...
                        , cReference =
                            Just
                                ColumnReference
                                    { crTableName = refTable
                                    , crConstraintName = refConstr
                                    , crFieldCascade = cfc
                                    }
                        } <-
                        newcols
            ...
            let
                foreignsAlt =
                    map
                        ( \fdef ->
                            let
                                (childfields, parentfields) = unzip (map (\((_, b), (_, d)) -> (b, d)) (foreignFields fdef))
                             in
                                AlterColumn
                                    name
                                    ( AddReference
        ...
        sqlbackend =
            mkSqlBackend
                MkSqlBackendArgs
                    ...
                    , connEscapeTableName =
                        T.pack . escapeDBName . T.unpack . unEntityNameDB . getEntityDBName
                    , connEscapeRawName = T.pack . escapeDBName . T.unpack
