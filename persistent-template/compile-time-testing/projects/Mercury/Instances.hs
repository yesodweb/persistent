...
instance PersistField Value where
  toPersistValue _value = undefined -- PersistText $ TE.encodeUtf8 $ J.encode value
  ...
  --   Right v -> Right v
  fromPersistValue (PersistByteString _bs) = undefined -- case J.eitherDecode (TE.encodeUtf8 bs) of
    -- Left s -> Left $ "Error decoding into Value; received " ++ TE.encodeUtf8 bs ++ " error: " ++ T.pack s
    -- Right v -> Right v
  fromPersistValue _x = undefined -- Left . T.pack $ "Value: When expecting PersistByteString/PersistText, received: " ++ show x
