...
import Data.Binary (Binary (..), Get, encode, getWord8)
...
instance Binary BinTimeOfDay where
    ...
    get = do
        let
            s = liftM unBinPico (Q.get :: Get BinPico)
        let
            tod = liftM3 TimeOfDay (Q.get :: Get Int) (Q.get :: Get Int) s
    ...
    get = do
        ...
        let
            pv = case tag of
                ...
                z ->
                    throw $
                        ParserError ("Incorrect tag " ++ show z ++ " came to Binary deserialization")
