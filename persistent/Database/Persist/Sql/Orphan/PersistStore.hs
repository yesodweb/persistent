      ...
      key <- case esql of
        ...
        ISRInsertGet sql1 sql2 -> do
            ...
            withRawQuery sql2 [] $ do
                mm <- CL.head
                let ks = case mm of
                        Just [PersistInt64 i]  -> [PersistInt64 i]
                        Just [PersistDouble i] -> [PersistInt64 $ truncate i] -- oracle need this!
                        ...
                        Just xs -> xs -- error $ "invalid sql2 return xs["++show xs++"] sql2["++show sql2++"] sql1["++show sql1++"]"
                        Nothing -> error $ "invalid sql2 returned nothing sql2["++show sql2++"] sql1["++show sql1++"]"
                case keyFromValues ks of
                    Right k -> return k
