...
class PersistStore m => PersistQuery m where
    ...
    update :: (PersistEntity val, EntityBackend val ~ MonadBackend m)
    ...
    updateGet :: ( PersistEntity val
                , MonadBackend m ~ EntityBackend val
                , Show (Key val))
    ...
    updateWhere :: (PersistEntity val, EntityBackend val ~ MonadBackend m)
                => [Filter val] -> [Update val] -> m ()

    -- | Delete all records matching the given criterion.
    deleteWhere :: (PersistEntity val, EntityBackend val ~ MonadBackend m)
                => [Filter val] -> m ()

    -- | Get all records matching the given criterion in the specified order.
    -- Returns also the identifiers.
    selectSource
           :: (PersistEntity val, EntityBackend val ~ MonadBackend m)
           => [Filter val]
           -> [SelectOpt val]
           -> C.Source m (Entity val)

    -- | get just the first record for the criterion
    selectFirst :: (PersistEntity val, EntityBackend val ~ MonadBackend m)
                => [Filter val]
                -> [SelectOpt val]
                -> m (Maybe (Entity val))
    ...
    selectKeys :: (PersistEntity record, EntityBackend record ~ MonadBackend m)
               => [Filter record]
               -> [SelectOpt record]
               -> C.Source m (Key record)

    -- | The total number of records fulfilling the given criterion.
    count :: (PersistEntity record, EntityBackend record ~ MonadBackend m)
          => [Filter record] -> m Int

-- | Call 'selectSource' but return the result as a list.
selectList :: (PersistEntity val, PersistQuery m, EntityBackend val ~ MonadBackend m)
           => [Filter val]
           -> [SelectOpt val]
           -> m [Entity val]
...
selectKeysList :: (PersistEntity record, PersistQuery m, EntityBackend record ~ MonadBackend m)
               => [Filter record]
               -> [SelectOpt record]
               -> m [Key record]
