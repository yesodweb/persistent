...
runMigration :: (MonadIO m, MBCIO m, MonadLogging m)
...
runMigrationSilent :: (MBCIO m, MonadIO m, MonadLogging m)
                   => Migration (SqlPersist m)
                   -> SqlPersist m [Text]
runMigrationSilent m = runMigration' m True

runMigration'
    :: (MBCIO m, MonadIO m, MonadLogging m)
    => Migration (SqlPersist m)
    -> Bool -- ^ is silent?
    -> SqlPersist m [Text]
...
runMigrationUnsafe :: (MBCIO m, MonadIO m, MonadLogging m)
                   => Migration (SqlPersist m)
                   -> SqlPersist m ()
...
executeMigrate :: (MonadIO m, MonadLogging m) => Bool -> Text -> SqlPersist m Text
